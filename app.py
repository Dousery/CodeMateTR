...
class ForumPost(db.Model):
    ...
    is_admin_post = db.Column(db.Boolean, default=False)  # Admin tarafından atıldı mı?
    created_at = db.Column(db.DateTime, default=datetime.utcnow)
...
def get_forum_posts():
    ...
    for post in posts.items:
        ...
        posts_data.append({
            ...
            'is_admin_post': post.is_admin_post,  # Admin gönderisi mi?
...
def admin_dashboard():
    ...
    try:
        ...
        # İlgi alanlarına göre dağılım
        interest_stats = db.session.query(
            User.interest, 
            db.func.count(User.id)
        ).filter(User.interest.isnot(None)).group_by(User.interest).all()
        
        interest_data = [{'interest': interest, 'count': count} for interest, count in interest_stats]
        ...
        return jsonify({
            ...
            'interest_distribution': interest_data
        })
        
    except Exception as e:
        return jsonify({'error': f'Admin dashboard hatası: {str(e)}'}), 500
...
def admin_toggle_user_admin(user_id):
    """Kullanıcının admin yetkisini açar/kapatır"""
    try:
        user = User.query.get_or_404(user_id)
        
        # Kendini admin yapamaz
        if user.username == session['username']:
            return jsonify({'error': 'Kendinizi admin yapamazsınız.'}), 400
        
        user.is_admin = not user.is_admin
        db.session.commit()
        
        return jsonify({
            'message': f'Kullanıcı {user.username} {"admin yapıldı" if user.is_admin else "admin yetkisi kaldırıldı"}.',
            'is_admin': user.is_admin
        })
        
    except Exception as e:
        return jsonify({'error': f'Admin yetkisi değiştirme hatası: {str(e)}'}), 500

@app.route('/admin/posts', methods=['GET'])
@admin_required
def admin_get_posts():
    """Tüm forum gönderilerini listeler (admin için)"""
    try:
        page = request.args.get('page', 1, type=int)
        per_page = request.args.get('per_page', 20, type=int)
        ...
        return jsonify({'error': f'Gönderi listesi hatası: {str(e)}'}), 500

@app.route('/admin/posts/<int:post_id>/delete', methods=['DELETE'])
@admin_required
def admin_delete_post(post_id):
    """Admin olarak forum gönderisini siler"""
    try:
        post = ForumPost.query.get_or_404(post_id)
...
def admin_get_reports():
    """Rapor edilmiş içerikleri listeler"""
    try:
        page = request.args.get('page', 1, type=int)
        per_page = request.args.get('per_page', 20, type=int)
        ...
        reports = query.order_by(ForumReport.created_at.desc()).paginate(
            page=page, 
            per_page=per_page, 
            error_out=False
        )
        ...
        return jsonify({'error': f'Rapor listesi hatası: {str(e)}'}), 500
        ...
        if action == 'dismiss':
            ...
            if report.post_id:
                ...
                if post:
                    ...
                    ForumLike.query.filter_by(post_id=post.id).delete()
                    db.session.delete(post)
            elif report.comment_id:
                comment = ForumComment.query.get(report.comment_id)
                if comment:
                    ForumLike.query.filter_by(comment_id=comment.id).delete()
                    db.session.delete(comment)
        elif action == 'warn_user':
            report.status = 'resolved'
            # Kullanıcıya uyarı gönder (notification)
            try:
                notification = ForumNotification(
                    username=report.reported_username,
                    notification_type='warning',
                    title='İçerik Uyarısı',
                    message=f'İçeriğiniz rapor edildi ve uyarı aldınız. Sebep: {report.reason}',
                    related_post_id=report.post_id,
                    related_comment_id=report.comment_id
                )
                db.session.add(notification)
            except Exception as e:
                print(f"Warning notification error: {e}")
        
        report.moderator_username = session['username']
        report.resolved_at = datetime.utcnow()
        db.session.commit()
        
        return jsonify({'message': 'Rapor başarıyla çözüldü.'})
        
    except Exception as e:
        db.session.rollback()
        return jsonify({'error': f'Rapor çözme hatası: {str(e)}'}), 500
...
def admin_send_notification():
    ...
    try:
        ...
        target_users = data.get('target_users', 'all')  # all, specific_interest, specific_users
        ...
        elif target_users == 'specific_interest':
            interest = data.get('interest')
            if not interest:
                return jsonify({'error': 'İlgi alanı gerekli.'}), 400
            users = User.query.filter_by(interest=interest).all()
        elif target_users == 'specific_users':
            usernames = data.get('usernames', [])
            if not usernames:
                return jsonify({'error': 'Kullanıcı adları gerekli.'}), 400
            users = User.query.filter(User.username.in_(usernames)).all()
        else:
            return jsonify({'error': 'Geçersiz hedef kullanıcı türü.'}), 400
        ...
        return jsonify({
            'message': f'{len(notifications)} kullanıcıya bildirim gönderildi.',
...
@app.route('/admin/forum/create-post', methods=['POST'])
@admin_required
def admin_create_forum_post():
    """Admin olarak forum gönderisi oluşturur"""
    try:
        data = request.json
        title = data.get('title')
        content = data.get('content')
        ...
        if not title or not content or not interest:
            return jsonify({'error': 'Başlık, içerik ve ilgi alanı gerekli.'}), 400
        ...
        new_post = ForumPost(
            ...
            post_type=post_type,
            tags=json.dumps(tags),
            is_admin_post=True  # Admin tarafından atıldığını belirt
        )
        ...
        return jsonify({
            'message': 'Admin gönderisi başarıyla oluşturuldu.',
            'post_id': new_post.id
        }), 201
